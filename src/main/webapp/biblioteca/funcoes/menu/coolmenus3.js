...
function cm_writeIt(text, tooltip){if(!this.img1){if(bw.ns4){this.ref.write('<span title="'+tooltip+'">'+text+'</span>');
this.ref.close()}else this.evnt.innerHTML='<span title="'+tooltip+'">'+text+'</span>'}}; function cm_moveY(y){this.y=y; this.css.top=y}
...
Making menu object
*************/
function cm_makeMenu(name,parent,text,link,target,width,height,img1,img2,bgcoloroff,bgcoloron,textcolor,hovercolor,onclick,onmouseover,onmouseout,tooltip){  // LLD
	...
	obj.ttip=(tooltip==void(0)||tooltip=='')?"":tooltip; // LLD
...
function cm_showSubs(name,l,num,cl){
	...
	for(i=0;i<this.l[l].maxnum;i++){
		...
		if(i<ln){
			var n=this.m[name].subs[i]; obj.aname=n; if(!bw.ns4||!this.NS4hover) obj.writeIt(this.m[n].text, this.m[n].ttip) // LLD
...
function cm_checkFolder(tmp,url){
	var urlTolow=url.toLowerCase();	if(urlTolow.indexOf("mailto:")>-1 || urlTolow.indexOf("/")==0 || urlTolow.indexOf("http://")==0 || urlTolow.indexOf("https://")==0 || urlTolow.indexOf("javascript:")==0) return url
