...
AjaxAnywhere.prototype.submitAJAX = function(additionalPostData, submitButton) {
    ...
    if (zones != "")
    {
		...
		postData = 'aazones=' + encodeURIComponent(zones);
		if (tempPostData != "")
		{
			if (tempPostData.charAt(0) == "&")
	...
	{
		if (additionalPostData.charAt(0) == "&")
...
AjaxAnywhere.prototype.getAJAX = function(url, zonesToRefresh) {
	...
	if (url.indexOf("aaxmlrequest") == -1)
	{
	    url += (url.indexOf("?") != -1) ? "&" : "?";
    	url += "aaxmlrequest=true"; // ajax request
	}

	// alert ('url = ' + url);
    // Número randômico - avoid caching
    idxRand = url.indexOf("aa_rand");
	if (idxRand != -1)
	{
		// Remover o número existente
		urlTemp = url.substring(0, idxRand-1);
		// alert ('urlTemp1=' + urlTemp);
		delimitador = url.indexOf("&", idxRand + 1);
		if (delimitador != -1)
		{
			urlTemp += url.substring(delimitador);
			// alert ('urlTemp2=' + urlTemp + '\nsufixo=' + url.substring(delimitador));
		}
		// alert ('urlTempFinal=' + urlTemp);
		url = urlTemp;
	}
   	url += "&aa_rand=" + Math.random();

	if (url.indexOf("aazones=") == -1 && zonesToRefresh != null && zonesToRefresh != "")
