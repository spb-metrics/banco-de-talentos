...
public class FacadeDelimitadora implements DelimitadoraSessaoTransacao
{
    /**
    ...
    public String obterPrimeira()
    {
        ...
        {
            if (stackTrace[i].getClassName().indexOf("Facade") > -1)
            {
                retorno = stackTrace[i].getClassName() + "." + stackTrace[i].getMethodName();
                break;
            }
        }
    ...
    public List obterTodas()
    {
        ...
        for (i = 3; i < tamanhoStackTrace; i++)
        {
            if (stackTrace[i].getClassName().indexOf("Facade") > -1)
            {
                retorno.add(stackTrace[i].getClassName() + "." + stackTrace[i].getMethodName());
                break;
            }
        }
        ...
        for (++i; i < tamanhoStackTrace; i++)
        {
            if (stackTrace[i].getClassName().indexOf("Facade") > -1)
            {
                if ((stackTrace[i].getClassName() + "." + stackTrace[i].getMethodName()).equals(retorno.get(0)))
                {
                    retorno.add(stackTrace[i].getClassName() + "." + stackTrace[i].getMethodName());
                }
                break;
