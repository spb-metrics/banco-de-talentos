...
import br.gov.camara.visao.bancotalentosgestao.sequencia.CurriculoSequencia;

/**
 * Action CurriculoAction
 *
 * @version FALTA DOCUMENTACAO
 * @author FALTA DOCUMENTACAO
 */

public final class CurriculoAction extends SigespAction
{
    // Inicializa o log

    private static Log log = LogFactory.getLog(CurriculoAction.class);
            ...
            HttpServletResponse response)
    {
        ...
        try
        {
            ...
            CurriculoSequencia curriculoSequencia = new CurriculoSequencia (mapping, form, request, response, (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_ERROS), (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_MENSAGENS), this, mapRecursos);
        	
        // Efetuar as operacoes do mapeamento '/curriculoGerarRelatorio'
        
        if ("/curriculoGerarRelatorio".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/curriculoGerarRelatorio'
            arlMensagens = curriculoSequencia.curriculoGerarRelatorio();
        }
     
        // Efetuar as operacoes do mapeamento '/curriculoPrepararVisualizacao'
        
        if ("/curriculoPrepararVisualizacao".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/curriculoPrepararVisualizacao'
            arlMensagens = curriculoSequencia.curriculoPrepararVisualizacao();
        }
     
        // Efetuar as operacoes do mapeamento '/curriculoPrepararVisualizacaoConsulta'
        
        if ("/curriculoPrepararVisualizacaoConsulta".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/curriculoPrepararVisualizacaoConsulta'
            arlMensagens = curriculoSequencia.curriculoPrepararVisualizacaoConsulta();
        }
     
        // Efetuar as operacoes do mapeamento '/curriculoPrepararVisualizacaoEstatistica'
        
        if ("/curriculoPrepararVisualizacaoEstatistica".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/curriculoPrepararVisualizacaoEstatistica'
            arlMensagens = curriculoSequencia.curriculoPrepararVisualizacaoEstatistica();
        }
