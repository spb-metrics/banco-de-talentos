...
import br.gov.camara.visao.bancotalentosgestao.sequencia.ConsultaAtributoTalentoOpcaoSequencia;

/**
 * Action ConsultaAtributoTalentoOpcaoAction
 *
 * @version FALTA DOCUMENTACAO
 * @author FALTA DOCUMENTACAO
 */

public final class ConsultaAtributoTalentoOpcaoAction extends SigespAction
{
    // Inicializa o log

    private static Log log = LogFactory.getLog(ConsultaAtributoTalentoOpcaoAction.class);
            ...
            HttpServletResponse response)
    {
        ...
        try
        {
            ...
            ConsultaAtributoTalentoOpcaoSequencia consultaAtributoTalentoOpcaoSequencia = new ConsultaAtributoTalentoOpcaoSequencia (mapping, form, request, response, (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_ERROS), (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_MENSAGENS), this, mapRecursos);
        	
        // Efetuar as operacoes do mapeamento '/consultaAtributoTalentoOpcaoEfetuarConsulta'
        
        if ("/consultaAtributoTalentoOpcaoEfetuarConsulta".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/consultaAtributoTalentoOpcaoEfetuarConsulta'
            arlMensagens = consultaAtributoTalentoOpcaoSequencia.consultaAtributoTalentoOpcaoEfetuarConsulta();
        }
     
        // Efetuar as operacoes do mapeamento '/consultaAtributoTalentoOpcaoPrepararVisualizacao'
        
        if ("/consultaAtributoTalentoOpcaoPrepararVisualizacao".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/consultaAtributoTalentoOpcaoPrepararVisualizacao'
            arlMensagens = consultaAtributoTalentoOpcaoSequencia.consultaAtributoTalentoOpcaoPrepararVisualizacao();
        }
