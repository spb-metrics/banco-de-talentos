...
import br.gov.camara.visao.bancotalentosapoio.sequencia.FiltroConsultaGrupoSequencia;

/**
 * Action FiltroConsultaGrupoAction
 *
 * @version FALTA DOCUMENTACAO
 * @author FALTA DOCUMENTACAO
 */

public final class FiltroConsultaGrupoAction extends SigespAction
{
    // Inicializa o log

    private static Log log = LogFactory.getLog(FiltroConsultaGrupoAction.class);
            ...
            HttpServletResponse response)
    {
        ...
        try
        {
            ...
            FiltroConsultaGrupoSequencia filtroConsultaGrupoSequencia = new FiltroConsultaGrupoSequencia (mapping, form, request, response, (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_ERROS), (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_MENSAGENS), this, mapRecursos);
        	
        // Efetuar as operacoes do mapeamento '/filtroConsultaGrupoEfetuarAtualizacao'
        
        if ("/filtroConsultaGrupoEfetuarAtualizacao".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/filtroConsultaGrupoEfetuarAtualizacao'
            arlMensagens = filtroConsultaGrupoSequencia.filtroConsultaGrupoEfetuarAtualizacao();
            blnForcarRemocaoFormBean = true;
        }
     
        // Efetuar as operacoes do mapeamento '/filtroConsultaGrupoPrepararVisualizacao'
        
        if ("/filtroConsultaGrupoPrepararVisualizacao".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/filtroConsultaGrupoPrepararVisualizacao'
            arlMensagens = filtroConsultaGrupoSequencia.filtroConsultaGrupoPrepararVisualizacao();
        }
