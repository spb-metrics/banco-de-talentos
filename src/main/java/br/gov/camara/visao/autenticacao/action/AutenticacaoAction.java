...
package br.gov.camara.visao.autenticacao.action;
...
import br.gov.camara.visao.autenticacao.sequencia.AutenticacaoSequencia;

/**
 * Action AutenticacaoAction
 *
 * @version FALTA DOCUMENTACAO
 * @author FALTA DOCUMENTACAO
 */

public final class AutenticacaoAction extends SigespAction
{
    // Inicializa o log

    private static Log log = LogFactory.getLog(AutenticacaoAction.class);
            ...
            HttpServletResponse response)
    {
        ...
        try
        {
            ...
            AutenticacaoSequencia autenticacaoSequencia = new AutenticacaoSequencia (mapping, form, request, response, (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_ERROS), (ActionMessages) mapMensagens.get(ActionMessagesUtil.CHAVE_REQUEST_MENSAGENS), this, mapRecursos);
        	
        // Efetuar as operacoes do mapeamento '/encaminharLogoff'
        
        if ("/encaminharLogoff".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/encaminharLogoff'
            arlMensagens = autenticacaoSequencia.encaminharLogoff();
        }
     
        // Efetuar as operacoes do mapeamento '/encaminharLogon'
        
        if ("/encaminharLogon".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/encaminharLogon'
            arlMensagens = autenticacaoSequencia.encaminharLogon();
        }
     
        // Efetuar as operacoes do mapeamento '/validarAutenticacaoExterna'
        
        if ("/validarAutenticacaoExterna".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/validarAutenticacaoExterna'
            arlMensagens = autenticacaoSequencia.validarAutenticacaoExterna();
        }
     
        // Efetuar as operacoes do mapeamento '/validarLogonEfetuarLogon'
        
        if ("/validarLogonEfetuarLogon".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/validarLogonEfetuarLogon'
            arlMensagens = autenticacaoSequencia.validarLogonEfetuarLogon();
        }
     
        // Efetuar as operacoes do mapeamento '/validarLogonEncaminharModulo'
        
        if ("/validarLogonEncaminharModulo".equals(mapping.getPath()))
        {
        
            // Efetuar a operacao referente ao mapeamento '/validarLogonEncaminharModulo'
            arlMensagens = autenticacaoSequencia.validarLogonEncaminharModulo();
        }
