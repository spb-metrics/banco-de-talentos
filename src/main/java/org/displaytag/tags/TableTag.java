...
public class TableTag extends HtmlTableTag
{
    ...
    /**
     * define se o pagesize foi definido na table ou se será utilizado o valor default
     * 
     * @version SIGESP
     */
    private boolean pagesizeDefinido;
    ...
    /**
     * @version SIGESP Nome do parâmetro que indica o número da página.
     */
    private String pageParamName;

    /**
     * Seta o nome do parâmetro que retorna o número da página
     * 
     * @param value nome do parâmetro de página.
     * @version SIGESP
     */
    public void setPageParamName(String value)
    {
        this.pageParamName = value;
    }
    ...
    public void setUid(String value)
    {
        ...
        {
            this.setHtmlId(value); // by default id is actually used for the html
            // id attribute, if no htmlId is added
        }
    ...
    public void addColumn(HeaderCell column)
    {
        ...
        {
            log.debug("[" + this.getUid() + "] addColumn " + column);
        }
    ...
    protected boolean isFirstIteration()
    {
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] first iteration=" + (this.rowNumber == 1) + " (row number=" + this.rowNumber + ")");
        }
    ...
    public int doStartTag() throws JspException
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doStartTag called");
        }
        ...
        // SIGESP
        if (!this.pagesizeDefinido)
        {
            this.pagesize = this.properties.getPagingPagesize();
        }

        if (this.pageContext.getRequest().getParameter(TableTagParameters.PARAMETER_EXPORTING) != null)
        {
            this.pagesize = 0;
            this.exporting = true;
        }
        ...
        if (this.currentMediaType != null && (previousMediaType == null || MediaTypeEnum.HTML.equals(previousMediaType)))
        {
            if (log.isDebugEnabled())
            {
                log.debug("[" + this.getUid() + "] setting media [" + this.currentMediaType + "] in this.pageContext");
            }
    ...
    public int doAfterBody()
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doAfterBody called - iterating on row " + this.rowNumber);
        }
    ...
    protected int doIteration()
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doIteration called");
        }
        ...
        if (this.tableIterator.hasNext())
        {
            ...
            if (this.getUid() != null)
            {
                if ((iteratedObject != null))
                {
                    ...
                    if (log.isDebugEnabled())
                    {
                        log.debug("[" + this.getUid() + "] setting attribute \"" + this.getUid() + "\" in pageContext");
                    }
                ...
                this.pageContext.setAttribute(this.getUid() + TableTagExtraInfo.ROWNUM_SUFFIX, new Integer(this.rowNumber));
            }
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doIteration() - iterator ended after " + (this.rowNumber - 1) + " rows");
        }
    ...
    private void initParameters() throws ObjectLookupException, FactoryInstantiationException
    {
        ...
        Integer pageNumberParameter = requestHelper.getIntParameter(this.getPageParamName());
        this.pageNumber = (pageNumberParameter == null) ? 1 : pageNumberParameter.intValue();

        Integer sortColumnParameter = requestHelper.getIntParameter(this.encodeParameter(TableTagParameters.PARAMETER_SORT));
        ...
        SortOrderEnum paramOrder = SortOrderEnum.fromCode(requestHelper.getIntParameter(this.encodeParameter(TableTagParameters.PARAMETER_ORDER)));
        ...
        Integer exportTypeParameter = requestHelper.getIntParameter(this.encodeParameter(TableTagParameters.PARAMETER_EXPORTTYPE));
        ...
        String fullName = this.getFullObjectName();
        ...
        {
            this.filteredRows = new LongRange(1, Long.MAX_VALUE);
        }
    ...
    public int doEndTag() throws JspException
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doEndTag called");
        }
        ...
        if (!this.doAfterBodyExecuted)
        {
            if (log.isDebugEnabled())
            {
                log.debug("[" + this.getUid() + "] tag body is empty.");
            }
        ...
        else if (!MediaTypeEnum.HTML.equals(this.currentMediaType))
        {
            if (log.isDebugEnabled())
            {
                log.debug("[" + this.getUid() + "] doEndTag - exporting");
            }
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] doEndTag - end");
        }
        ...
        if (this.tableIterator.hasNext())
        {
            ...
            while (propertiesIterator.hasNext())
            {
                ...
                if (!"class".equals(propertyName)) //$NON-NLS-1$
                {
                    ...
                    // handle title i18n
                    headerCell.setTitle(this.properties.geResourceProvider().getResource(null, propertyName, this, this.pageContext));
    ...
    protected int doExport() throws JspException
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] currentMediaType=" + this.currentMediaType);
        }
    ...
    protected void setupViewableData()
    {
        ...
        {
            throw new RuntimeException("If you set the totalRecords property, you won't be able to sort by list, only by table");
        }
        ...
        if (this.pagesize > 0)
        {
            ...
            this.listHelper = new SmartListHelper(fullList, fullList.size(), this.pagesize, this.properties, totalRecordsParam);
    ...
    private void writeHTMLData() throws JspException
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] getHTMLData called for table [" + this.getUid() + "]");
        }
        ...
        // SIGESP
        if (this.properties.getAddPagingBannerTop() != 9)
        {
            // search result and navigation bar
            // SIGESP
            this.writeSearchResultAndNavigation(this.properties.getAddPagingBannerTop());
        }
        ...
        if (this.footer != null)
        {
            this.write(TagConstants.TAG_TFOOTER_OPEN, out);
            this.write(this.footer, out);
            this.write(TagConstants.TAG_TFOOTER_CLOSE, out);
            // reset footer
            this.footer = null;
        }

        // open table body
        this.write(TagConstants.TAG_TBODY_OPEN, out);

        // write table body
        this.writeTableBody();

        // close table body
        this.write(TagConstants.TAG_TBODY_CLOSE, out);
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] getHTMLData end");
        }
    }
    ...
    private void writeTableHeader()
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] getTableHeader called");
        }

        // open thead
        this.write(TagConstants.TAG_THEAD_OPEN, out);

        // open tr
        this.write(TagConstants.TAG_TR_OPEN, out);

        // no columns?
        if (this.tableModel.isEmpty())
        {
            this.write(TagConstants.TAG_TH_OPEN, out);
            this.write(TagConstants.TAG_TH_CLOSE, out);
        }
        ...
        while (iterator.hasNext())
        {
            ...
            if (this.exporting && !headerCell.getExport())
            {
                continue;
            }
            ...
            // SIGESP
            HtmlAttributeMap headerAttributes = headerCell.getHeaderAttributes();
            if (headerAttributes.containsKey("titleAlign"))
            {
                if (headerAttributes.containsKey("align"))
                {
                    headerAttributes.remove("align");
                    headerAttributes.put("align", headerAttributes.get("titleAlign"));
                    headerAttributes.remove("titleAlign");
                }
                else
                {
                    headerAttributes.put("align", headerAttributes.get("titleAlign"));
            ...
            else
            {
                ...
                {
                    headerAttributes.remove("align");
                    headerAttributes.put("align", "center");
                }
                else
                {
                    headerAttributes.put("align", "center");
        ...
        // close tr
        this.write(TagConstants.TAG_TR_CLOSE, out);

        // close thead
        this.write(TagConstants.TAG_THEAD_CLOSE, out);

        if (log.isDebugEnabled())
        {
            log.debug("[" + this.getUid() + "] getTableHeader end");
        }
    }
    ...
    private Href getSortingHref(HeaderCell headerCell)
    {
        ...
        href.addParameter(this.encodeParameter(TableTagParameters.PARAMETER_SORT), headerCell.getColumnNumber());
        ...
        href.addParameter(this.encodeParameter(TableTagParameters.PARAMETER_ORDER), sortOrderParam);

        // If user want to sort the full table I need to reset the page number.
        if (this.tableModel.isSortFullTable())
        {
            // href.addParameter(encodeParameter(TableTagParameters.PARAMETER_PAGE),
            // 1);
            // SIGESP
            href.addParameter(this.encodeParameter(this.getPageParamName()), 1);
        }
    ...
    private void writeTableBody() throws ObjectLookupException, DecoratorException
    {
        ...
        while (rowIterator.hasNext())
        {
            ...
            if (log.isDebugEnabled())
            {
                log.debug("[" + this.getUid() + "] rowIterator.next()=" + row);
            }
            if (this.tableModel.getTableDecorator() != null)
            {
            ...
            if (log.isDebugEnabled())
            {
                log.debug("[" + this.getUid() + "] creating ColumnIterator on " + this.tableModel.getHeaderCellList());
            }
            ...
            while (columnIterator.hasNext())
            {
                ...
                if (this.exporting && !column.getExport())
                {
                    continue;
                }
            ...
            if (this.tableModel.isEmpty())
            {
                if (log.isDebugEnabled())
                {
                    log.debug("[" + this.getUid() + "] table has no columns");
                }
                this.write(TagConstants.TAG_TD_OPEN, out);
                this.write(row.getObject().toString(), out);
                this.write(TagConstants.TAG_TD_CLOSE, out);
            }
        ...
        {
            this.write(MessageFormat.format(this.properties.getEmptyListRowMessage(), new Object[] { new Integer(this.tableModel.getNumberOfColumns(this.exporting)) }), out);
        }
    }
    ...
    private void writeTableFooter()
    {
        // Put the page stuff there if it needs to be there...
        // SIGESP
        if (this.properties.getAddPagingBannerBottom() != 9)
        {
            this.writeSearchResultAndNavigation(this.properties.getAddPagingBannerBottom());
        }
     ...
     */
    private void writeSearchResultAndNavigation(int tipo)
    {
        if (this.pagesize != 0 && this.listHelper != null)
        {
            ...
            { // summary and navigation
                ...
                this.write(this.listHelper.getPageNavigationBar(navigationHref,
                // SIGESP
                /* encodeParameter( */this.getPageParamName())/* ) */);
            }
            else if (tipo == 1)
            { // summary
                this.write(this.listHelper.getSearchResultsSummary());
    ...
    private void writeExportLinks()
    {
        ...
        while (iterator.hasNext())
        {
            ...
            if (this.properties.getAddExport(currentExportType))
            {
                ...
                exportHref.addParameter(this.encodeParameter(TableTagParameters.PARAMETER_EXPORTTYPE), currentExportType.getCode());
    ...
    /**
     * Returns o nome do parâmetro de página
     * 
     * @return String
     * @version SIGESP
     */
    ...
    {
        String strRetorno = TableTagParameters.PARAMETER_PAGE;
        if (this.pageParamName != null)
        {
            strRetorno = this.pageParamName;
        }
        return strRetorno;
    }
