...
public abstract class TemplateTag extends BodyTagSupport
{
    ...
    protected Object evaluateExpression(String expression) throws ObjectLookupException
    {
        ...
        // verifica cada um dos escopos...
        // SIGESP
        int[] scopes = {PageContext.APPLICATION_SCOPE, PageContext.PAGE_SCOPE, PageContext.SESSION_SCOPE};
        int i = 0;

        if (expression.equals(expressionWithoutScope)) {
        	while (LookupUtil.getBeanValue(this.pageContext, expressionWithoutScope, scope) == null && i <= 2) {
        		scope = scopes[i];
        		i++;
        	}
        }
