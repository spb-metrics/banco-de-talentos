...
public class TableModel
{
     ...
     */
    public int getNumberOfColumns(boolean exporting)
    {
        int numColunas = 0;

        if (exporting)
        {
            Iterator itrColumns = this.headerCellList.iterator();

            while (itrColumns.hasNext())
            {
                HeaderCell headerCell = (HeaderCell) itrColumns.next();
                if (!exporting || headerCell.getExport())
                {
                    numColunas++;
                }
            }
        ...
        {
            numColunas = this.headerCellList.size();
        }
    ...
    private void sortRowList(List list)
    {
        if (this.isSorted())
        {
            ...
            if (sortedHeaderCell != null)
            {
                ...
                if (sortedHeaderCell.getBeanPropertyName() != null || (this.sortedColumn != -1 && this.sortedColumn < this.headerCellList.size()))
                {
    ...
    public String toString()
    {
        ...
        .append("numberOfColumns", this.getNumberOfColumns(false)) //$NON-NLS-1$
