...
import sigesp.comum.util.biblioteca.Conversao;
    ...
    public String createChoppedAndLinkedValue() throws ObjectLookupException, DecoratorException
    {
        ...
        if (this.header.getHref() != null)
        {
            ...
            if (this.header.getHrefType() != null && this.header.getHrefType().equals("replace"))
            {
                ...
                {
                    stbNovaString.replace(intPosicao, intPosicao + this.header.getParamName().length(), strValor.toString());
                }
                ...
                {
                    stbNovaString.replace(intPosicaoIgual - 1, intPosicaoIgual - 1 + stbNovaString.substring(intPosicaoIgual - 1).indexOf(">") - 1, ";");
                ...
                {
                    throw new RuntimeException("Os parâmetros informados na display-tag estão incorretos");
                }
